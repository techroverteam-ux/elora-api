...
// Debug FTP connection and upload process
async function debugFTPFlow() {
  const API_BASE = 'https://elora-api-smoky.vercel.app/api/v1';
  
  console.log('🔍 Debugging FTP Connection & Upload Process\n');
  ...
  try {
    ...
    formData.append('notes', 'Debug FTP test submission');
    ...
    console.log('🚀 Submitting recce data...');
    ...
    try {
      ...
      console.error('❌ Recce submission failed:');
      console.error('Status:', uploadError.response?.status);
      console.error('Message:', uploadError.response?.data?.message || uploadError.message);
      console.error('Full error:', uploadError.response?.data);
    }
